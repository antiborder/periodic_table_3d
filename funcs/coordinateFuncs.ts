...
const getCoordinate = (count: number, atomicNumber: number): Num3 => {
  ...
  const radius0 = (atomicNumber: number): number => {return cartesianToCylindrical(getTablePosition(atomicNumber))[0]}
  const theta0 = (atomicNumber: number): number => {return cartesianToCylindrical(getTablePosition(atomicNumber))[1]}
  const z0 = (atomicNumber: number): number => {return cartesianToCylindrical(getTablePosition(atomicNumber))[2]}
      ...
      case 3:
        baseTheta =
          elements[atomicNumber].tableColumn <= 2
            ? ((elements[atomicNumber].tableColumn - 3.5) / getSpiralPeriod(atomicNumber)) * 2 * Math.PI
            : ((elements[atomicNumber].tableColumn - 13.5) / getSpiralPeriod(atomicNumber)) * 2 * Math.PI
        break
      case 4:
      case 5:
      ...
      case 7:
        baseTheta =
          elements[atomicNumber].tableColumn <= 2
            ...
            : ((elements[atomicNumber].tableColumn + 4.5) / getSpiralPeriod(atomicNumber)) * 2 * Math.PI
        break
      case 9:
      case 10:
        baseTheta = ((elements[atomicNumber].tableColumn - 9.5) / getSpiralPeriod(atomicNumber)) * 2 * Math.PI
  ...
  const getTransition4to5Coordinate = (t: number, atomicNumber: number): Num3 => {
    ...
    return [
      orbitalPosition(atomicNumber)[0]+ (t - 4) * (tablePosition[0] - orbitalPosition(atomicNumber)[0]),
      orbitalPosition(atomicNumber)[1]+ (t - 4) * (tablePosition[1] - orbitalPosition(atomicNumber)[1]),
      orbitalPosition(atomicNumber)[2]+ (t - 4) * (tablePosition[2] - orbitalPosition(atomicNumber)[2]),
    ]
  }
  ...
  const getRotationAngle = (t: number, atomicNumber: number): Num3 => {
    t = t % constants.NUMBER_OF_SHAPES
      if (t >= 0 && t <= 1) {
          ...
          const baseTheta = theta2(atomicNumber) + Math.pow((t - 2), 1 / 2) * (getTheta3down(atomicNumber) - theta2(atomicNumber))
          return [0, 0, baseTheta + Math.PI * 1.1 / 2]
      } else if (t > 3 && t <= 4) {
          const revaluedTheta0 = theta0

          return [0, 0, revalueTheta(getTheta3up(atomicNumber) + Math.pow(t - 3, 2) * (revaluedTheta0(atomicNumber) - getTheta3up(atomicNumber)) + Math.PI * 1.1 / 2) * (4 - t)]
